...
Embedding vectors are loaded from local CSV files when present; when CSVs are
absent they are fetched live from TigerGraph via the REST++ API.
"""
...
with tab_merch:
    ...
    with col_b:
        ...
        fig_sage.update_layout(height=460, showlegend=True,
...
with tab_user:
    ...
    with st.spinner("Computing GraphSAGE user embeddings + UMAP (~20 s first run)…"):
        sage_user = compute_sage_user_embeddings(
            user_embs, txn_risk, user_to_txn, alpha=alpha
        )
        # Build UMAP dataframes for users
    ...
    with col_a:
        fig_ou = px.scatter(orig_udf, x="x", y="y", color="region" if "region" in orig_udf.columns else None,
    ...
    with col_b:
        fig_su = px.scatter(sage_udf, x="x", y="y", color="region" if "region" in sage_udf.columns else None,
                             hover_name="entity_id", hover_data=hcols,
                             color_discrete_sequence=px.colors.qualitative.Bold,
...
with tab_cold:
    ...
    st.markdown(
        """
        This is the **signature GraphSAGE demonstration**.
    ...
    def _add_star(fig: go.Figure, df: pd.DataFrame, key: str) -> go.Figure:
    ...
    with c1:
        ...
        fig_o = _add_star(fig_o, orig_cs, "orig")
    ...
    with c2:
        ...
        fig_s = _add_star(fig_s, sage_cs, "sage")
    ...
    meta_idx = meta_merch.set_index("merchant_id") if "merchant_id" in meta_merch.columns else pd.DataFrame()

    def _nn_df(nn: list[tuple[str, float]]) -> pd.DataFrame:
        rows = []
        for mid, sim in nn:
            name = meta_idx.at[mid, "merchant_name"] if (not meta_idx.empty and mid in meta_idx.index and "merchant_name" in meta_idx.columns) else mid
            cat  = meta_idx.at[mid, "merchant_category_text"] if (not meta_idx.empty and mid in meta_idx.index and "merchant_category_text" in meta_idx.columns) else "—"
            city = meta_idx.at[mid, "merchant_city"] if (not meta_idx.empty and mid in meta_idx.index and "merchant_city" in meta_idx.columns) else "—"
            rows.append({
                "Merchant": name,
                "City":     city,
                "Category": cat,
    ...
    with col_nn2:
        ...
        st.caption("Positioned by risk context → neighbours are the merchants most similar in risk profile")
...
with tab_explain:
    ...
    # Build display labels: "Merchant Name (ID)"
    probe_meta = meta_merch.set_index("merchant_id") if "merchant_id" in meta_merch.columns else pd.DataFrame()
    ...
    if chosen:
        ...
        st.caption(f"Selected: **{chosen_name}**")
        ...
        # Show embedding shift magnitude
...
with tab_2hop:
    ...
    with st.spinner("Computing 1-hop and 2-hop embeddings + UMAP (~30 s first run)…"):
        @st.cache_data(show_spinner=False)
        def _2hop_data(_merch, _user, _txn, _m2t, _t2u, _u2t, _alpha, _nn, _md):
            sage_1h = compute_sage_merchant_embeddings(_merch, _txn, _m2t, alpha=_alpha)
            sage_2h = compute_sage_merchant_2hop(_merch, _user, _txn, _m2t, _t2u, _u2t, alpha=_alpha)
            # UMAP on all three sets combined
            from umap import UMAP
            ids   = sorted(_merch.keys())
            mat_o = np.array([_merch[i]  for i in ids], dtype=np.float32)
            mat_1 = np.array([sage_1h[i] for i in ids], dtype=np.float32)
            mat_2 = np.array([sage_2h[i] for i in ids], dtype=np.float32)
            combined = np.vstack([mat_o, mat_1, mat_2])
            coords   = UMAP(n_components=2, n_neighbors=_nn, min_dist=_md,
                            random_state=42, low_memory=True).fit_transform(combined)
            n = len(ids)
            return ids, coords[:n], coords[n:2*n], coords[2*n:], sage_1h, sage_2h

        ids_2h, c_orig, c_1h, c_2h, sage_1h, sage_2h = _2hop_data(
    ...
    st.markdown("**What to look for:** clusters tighten further in 2-hop — merchants serving similar *user profiles* are pulled together, not just merchants sharing transactions.")
    ...
    for mid in ids_2h:
        ...
        shifts.append({"Merchant": name, "Category": cat, "Shift (1-hop)": round(s1, 4), "Shift (2-hop)": round(s2, 4), "Δ (2-hop − 1-hop)": round(s2 - s1, 4)})
    ...
    st.caption("Merchants with large Δ are most influenced by *user-level* risk propagation beyond what their own transactions reveal.")
